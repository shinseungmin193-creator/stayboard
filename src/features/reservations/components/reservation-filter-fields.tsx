...
}: {value: ReservationFilterState;onChange: (next: ReservationFilterState) => void;properties: ReservationPropertyOption[];rooms: ReservationRoomOption[];providers: ReservationProviderOption[];effectiveDateRange: {from: string;to: string;};compact?: boolean;}) {const i18n = useTranslations();const DATE_PRESETS: Array<{value: Exclude<ReservationDatePreset, "custom">;label: string;}> = [{ value: "today", label: i18n("common.today") }, { value: "this-week", label: i18n("auto.m0640") }, { value: "this-month", label: i18n("auto.m0641") }];
  ...
  return (
    <div className={cn("grid gap-5", compact && "lg:grid-cols-4 lg:gap-3")}>
      <div className="space-y-1.5">
        <Label htmlFor={compact ? "desktop-reservation-property" : "mobile-reservation-property"}>{i18n("common.property")}</Label>
        <select
      ...
      <div className={cn("space-y-1.5", !compact && "min-w-0")}>
        <Label htmlFor={compact ? "desktop-reservation-room" : "mobile-reservation-room"}>{i18n("common.room")}</Label>
        {compact ?
        ...
        <div className="space-y-2">
            <div className="relative"><Search className="pointer-events-none absolute top-3.5 left-3 size-4 text-muted-foreground" /><input id="mobile-reservation-room" value={roomQuery} onChange={(event) => setRoomQuery(event.target.value)} placeholder={currentRoom ? `${currentRoom.propertyName} · ${currentRoom.name}` : i18n("auto.m0427")} className={cn(fieldClassName, "pl-9")} /></div>
            <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border p-1" role="listbox" aria-label={i18n("auto.m0428")}>
              <button type="button" onClick={() => onChange({ ...value, roomId: null })} className={cn("flex min-h-10 w-full items-center justify-between rounded-md px-3 text-left text-sm hover:bg-muted", !value.roomId && "bg-muted font-semibold")}><span>{i18n("auto.m0081")}</span>{!value.roomId && <Check className="size-4" />}</button>
              ...
              {eligibleRooms.length === 0 && <p className="px-3 py-4 text-center text-xs text-muted-foreground">{i18n("auto.m0429")}</p>}
            </div>
          </div>
        }
      </div>
      ...
      <fieldset className={cn("space-y-2", compact && "lg:col-span-2")}>
        <legend className="text-sm font-medium">{i18n("technical.ota")}</legend>
      ...
      <fieldset className={cn("space-y-2", compact && "lg:col-span-2")}>
        <legend className="text-sm font-medium">{i18n("auto.m0430")}</legend>
        <div className="flex flex-wrap gap-2">
          <button type="button" aria-pressed={value.statuses.length === 0} onClick={() => onChange({ ...value, statuses: [] })} className={cn("min-h-10 rounded-full border px-3 text-xs font-semibold transition", value.statuses.length === 0 ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-muted")}>{i18n("auto.m0102")}</button>
          {STATUS_OPTIONS.map((status) => {const selected = value.statuses.includes(status);return <button key={status} type="button" aria-pressed={selected} onClick={() => onChange({ ...value, statuses: toggleValue(value.statuses, status) })} className={cn("min-h-10 rounded-full border px-3 text-xs font-semibold transition", selected ? getReservationStatusVariant(status) : "bg-background hover:bg-muted")}>{getLocalizedReservationStatusLabel(status, i18n)}</button>;})}
        </div>
      </fieldset>

      <div className={cn("space-y-2", compact && "lg:col-span-2")}>
        <Label htmlFor={compact ? "desktop-reservation-date-field" : "mobile-reservation-date-field"}>{i18n("auto.m0431")}</Label>
        <select id={compact ? "desktop-reservation-date-field" : "mobile-reservation-date-field"} value={value.dateField} onChange={(event) => onChange({ ...value, dateField: event.target.value as ReservationFilterState["dateField"] })} className={fieldClassName}>
          <option value="stay">{i18n("auto.m0432")}</option><option value="checkIn">{i18n("auto.m0433")}</option><option value="checkOut">{i18n("auto.m0434")}</option>
        </select>
      </div>

      <div className={cn("space-y-2", compact && "lg:col-span-2")}>
        <span className="text-sm font-medium">{i18n("auto.m0435")}</span>
