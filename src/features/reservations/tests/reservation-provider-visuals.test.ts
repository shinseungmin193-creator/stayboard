...
import {
  getProviderVisual,
  getReservationBarLabel,
  PROVIDER_VISUALS,
  RESERVATION_CONFLICT_VISUAL,
} from "../provider-visuals";

test("예약 막대에 OTA 이름을 표시한다", () => {
...
test("좁은 막대에는 축약된 Provider 이름을 표시한다", () => {
  assert.equal(getReservationBarLabel("BOOKING", 64), "Booking");
  assert.equal(getReservationBarLabel("AIRBNB", 24), "Airbnb");
});

test("알 수 없는 Provider는 번역 함수로 현지화한다", () => {
  const translate = (key: string, values?: { provider: string }) =>
    key === "provider.other" ? "기타" : `${values?.provider} 예약`;

  assert.equal(getReservationBarLabel("UNKNOWN", 120, translate), "기타 예약");
