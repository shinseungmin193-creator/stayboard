...
export function CleaningSection({
  ...
  return (
    <section className="space-y-3" aria-labelledby={`cleaning-section-${section}`}>
      ...
      {data.items.length > 0 ? <div className="space-y-2.5">{data.items.map((task) => <CleaningTaskCard key={task.id} task={task} role={role} currentUserId={currentUserId} referenceAt={referenceAt} timeZone={timeZone} locale={locale} pending={pendingTaskId === task.id} canManageCompletion={canManageCompletion} onOpenDetails={onOpenDetails} onOpenRoomNotes={onOpenRoomNotes} onCancelStart={onCancelStart} onEditCompletion={onEditCompletion} onRevertCompletion={onRevertCompletion} onWorkflow={onWorkflow} />)}</div> : <div className="rounded-2xl border border-dashed bg-muted/15 px-4 py-8 text-center text-sm text-muted-foreground">{t(`sections.empty.${section}`)}</div>}
    </section>
  );
}
