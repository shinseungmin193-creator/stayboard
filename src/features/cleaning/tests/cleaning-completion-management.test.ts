...
test("완료 수정·취소는 DEVELOPER/ADMIN/STAFF 공통 권한을 서버에서 검증한다", () => {
  ...
  assert.match(access, /STAFF:[\s\S]*PERMISSIONS\.CLEANING_COMPLETION_MANAGE/);
...
test("STAFF 완료 수정·취소도 task의 room 접근 범위를 서버에서 다시 검증한다", () => {
  const accessGuard = read("src/features/cleaning/server/cleaning-task-access.ts");
  const actions = read("src/features/cleaning/cleaning.actions.ts");
  assert.match(accessGuard, /const context = await requireRoomAccess\(task\.roomId, permission\)/);
  assert.match(actions, /updateCleaningCompletionAction[\s\S]*requireCleaningTaskAccess\(parsed\.data\.taskId, PERMISSIONS\.CLEANING_COMPLETION_MANAGE\)/);
  assert.match(actions, /revertCleaningCompletionAction[\s\S]*requireCleaningTaskAccess\(parsed\.data\.taskId, PERMISSIONS\.CLEANING_COMPLETION_MANAGE\)/);
});
...
test("완료 사진은 접근 범위 내 모든 역할이 추가·삭제할 수 있고 0장이 되어도 task 상태를 변경하지 않는다", () => {
  const service = read("src/features/cleaning/server/cleaning-task.service.ts");
...
test("완료 카드 메뉴와 반응형 dialog는 수정·destructive 완료 취소를 제공한다", () => {
  ...
  assert.match(workspace, /hasPermission\(role, PERMISSIONS\.CLEANING_COMPLETION_MANAGE\)/);
