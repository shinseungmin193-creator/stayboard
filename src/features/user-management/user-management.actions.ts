...
async function targetInfo(userId: string, context: AccessContext) {
  const target = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, isActive: true, systemRole: true, memberships: { select: { companyId: true, role: true } } } });
...
export async function createManagedUserAction(_state: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      const user = await tx.user.create({ data: { name: parsed.data.name, email: parsed.data.email, passwordHash, systemRole: "NONE", isActive: parsed.data.isActive } });
...
export async function setManagedUserActiveAction(_state: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await prisma.$transaction([prisma.user.update({ where: { id: target.id }, data: { isActive: parsed.data.isActive } }), prisma.auditLog.create({ data: { actorUserId: context.userId, targetUserId: target.id, action: parsed.data.isActive ? "USER_ACTIVATED" : "USER_DEACTIVATED", details: { before: { isActive: target.isActive }, after: { isActive: parsed.data.isActive } } } })]);
...
export async function resetManagedUserPasswordAction(_state: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  if (!target || !canManageTarget({ actorRole: context.role, actorUserId: context.userId, targetUserId: target.id, targetRole: target.targetRole, sameCompany: target.sameCompany })) return forbidden();
  const passwordHash = await hashPassword(parsed.data.password);
  await prisma.$transaction([prisma.user.update({ where: { id: target.id }, data: { passwordHash } }), prisma.auditLog.create({ data: { actorUserId: context.userId, targetUserId: target.id, action: "PASSWORD_RESET" } })]);
...
export async function updateManagedUserRoleAction(_state: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  if (!companyId || (context.role !== "DEVELOPER" && companyId !== context.activeCompanyId)) return forbidden();
  await prisma.$transaction(async (tx) => {
    await tx.user.update({ where: { id: target.id }, data: { systemRole: "NONE" } });
