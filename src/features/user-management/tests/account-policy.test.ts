...
test("자기 자신 보호와 ADMIN 회사 범위를 적용한다", () => {
  ...
  assert.equal(canManageTarget({ actorRole: "DEVELOPER", actorUserId: "developer-a", targetUserId: "developer-b", targetRole: "DEVELOPER", sameCompany: true }), false);
...
test("자기 자신과 모든 DEVELOPER 계정은 일반 관리 작업으로 변경하지 못한다", () => {
  ...
  assert.equal(canDeactivateAccount({ isSelf: false, targetRole: "DEVELOPER", activeDeveloperCount: 2 }), false);
  assert.equal(canChangeAccountRole({ isSelf: false, currentRole: "DEVELOPER", nextRole: "ADMIN", activeDeveloperCount: 1 }), false);
  assert.equal(canChangeAccountRole({ isSelf: false, currentRole: "DEVELOPER", nextRole: "ADMIN", activeDeveloperCount: 2 }), false);
});
