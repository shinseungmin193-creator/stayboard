...
test("통계 사용자 지정 기간은 3개월보다 오래된 날짜도 그대로 조회한다", () => { const result = resolveOccupancyPeriod({ from: "2025-09-01", to: "2025-09-30" }, day("2026-09-21")); assert.equal(result.key, "custom"); assert.equal(result.period.startLabel, "2025-09-01"); assert.equal(result.period.endLabel, "2025-09-30"); });
