...
export interface ExistingConflict extends ReservationConflictPair { id: string; status: "ACTIVE" | "RESOLVED" }
...
export function classifyConflicts(existing: ExistingConflict[], detected: ReservationConflictPair[]): ConflictClassification {
  ...
  for (const pair of detected) {
    ...
    else result.refresh.push({ id: previous.id, pair, reactivate: previous.status === "RESOLVED", overlapChanged: previous.overlapStart.getTime() !== pair.overlapStart.getTime() || previous.overlapEnd.getTime() !== pair.overlapEnd.getTime() });
  }
