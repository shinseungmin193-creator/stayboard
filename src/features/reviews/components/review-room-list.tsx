  ...
  return <>
    <div className="grid gap-3 md:hidden">
      {rooms.map((room) => <Card key={room.id}><CardContent className="space-y-4 p-4">
        ...
        <div className="grid gap-3">{providers.map((item) => <div key={item.provider} className="grid grid-cols-[110px_1fr] gap-2 rounded-lg border bg-muted/20 p-2.5"><p className="text-sm font-medium">{item.label}</p><ReviewSummaryStatus roomId={room.id} listing={room.listings.find((listing) => listing.provider === item.provider)} /></div>)}</div>
