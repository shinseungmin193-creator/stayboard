...
export function ReviewRoomDetailView({ room, selectedProvider, locale }: {
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-3">
      {REVIEW_PROVIDER_CONFIG.map((provider) => <Card key={provider.provider}><CardHeader className="pb-2"><CardTitle className="text-sm">{provider.label}</CardTitle></CardHeader><CardContent><ReviewSummaryStatus roomId={room.id} listing={room.listings.find((listing) => listing.provider === provider.provider)} /></CardContent></Card>)}
    </div>
