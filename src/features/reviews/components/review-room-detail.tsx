...
import { ReviewPlatformCell } from "./review-platform-cell";
...
export function ReviewRoomDetailView({ room, selectedProvider, locale }: {
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-3">
      {REVIEW_PROVIDER_CONFIG.map((provider) => <Card key={provider.provider}><CardHeader className="pb-2"><CardTitle className="text-sm">{provider.label}</CardTitle></CardHeader><CardContent><ReviewPlatformCell roomId={room.id} roomLabel={`${room.propertyName} ${room.name}`} provider={provider.provider} initialListing={room.listings.find((listing) => listing.provider === provider.provider)} /></CardContent></Card>)}
    </div>
    ...
    {!selected ? <Card><CardContent className="flex min-h-52 items-center justify-center text-sm text-muted-foreground">{t("reviews.states.providerMissing")}</CardContent></Card> : <div className="space-y-3">
      ...
      {!selected.reviews.length ? <Card><CardContent className="flex min-h-52 flex-col items-center justify-center gap-2 text-center"><Star className="size-7 text-muted-foreground" /><p className="font-medium">{selected.reviewCount === 0 ? t("reviews.states.noReviews") : t("reviews.states.noReviewContent")}</p>{selected.reviewCount !== 0 && <p className="text-sm text-muted-foreground">{t("reviews.states.noPublicContent")}</p>}</CardContent></Card> : selected.reviews.map((review) => <Card key={review.id}><CardContent className="space-y-2 p-4">
