...
export function classifyReservations(existing: ExistingReservation[], incoming: NormalizedReservation[], syncStartedAt: Date): ReservationClassification {
  ...
  incomingByUid.forEach((reservation, uid) => { const current = existingByUid.get(uid); if (!current) result.create.push(reservation); else if (reservationFieldsEqual(current, reservation)) result.unchanged.push(current); else result.update.push({ id: current.id, reservation }); });
  existing.forEach((reservation) => { if (reservation.createdAt <= syncStartedAt && !incomingByUid.has(reservation.rawUid) && reservation.status !== "CANCELLED") result.cancelIds.push(reservation.id); });
  return result;
}
