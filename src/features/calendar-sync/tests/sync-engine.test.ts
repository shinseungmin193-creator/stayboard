...
test("신규·수정·동일·누락 예약을 분류한다", () => { const current = existing(); const created = normalized({ rawUid: "new", providerReservationId: "new", summary: "New" }); const changed = normalized({ summary: "Changed" }); const missing = existing({ id: "missing", rawUid: "missing", providerReservationId: "missing" }); const same = existing({ id: "same", rawUid: "same", providerReservationId: "same" }); const incomingSame = normalized({ rawUid: "same", providerReservationId: "same" }); const result = classifyReservations([current, missing, same], [changed, created, incomingSame], new Date()); assert.equal(result.create.length, 1); assert.equal(result.update.length, 1); assert.equal(result.unchanged.length, 1); assert.deepEqual(result.cancelIds, ["missing"]); });
test("필드 변경과 CANCELLED 예약 재등장을 update로 분류한다", () => { assert.equal(reservationFieldsEqual(existing(), normalized({ summary: "Changed" })), false); const result = classifyReservations([existing({ status: "CANCELLED" })], [normalized({ status: "CONFIRMED" })], new Date()); assert.equal(result.update.length, 1); assert.equal(result.cancelIds.length, 0); });
test("이미 취소된 누락 예약은 다시 취소하지 않는다", () => { const result = classifyReservations([existing({ status: "CANCELLED" })], [], new Date()); assert.equal(result.cancelIds.length, 0); });
test("기존 예약이 없는 CANCELLED 이벤트는 새 Reservation으로 만들지 않는다", () => { const result = classifyReservations([], [normalized({ status: "CANCELLED" })], new Date()); assert.equal(result.create.length, 0); assert.equal(result.update.length, 0); });
