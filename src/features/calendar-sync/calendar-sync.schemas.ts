...
export const roomCalendarFilteredSyncSchema = bulkSyncCalendarSourcesSchema.pick({ propertyId: true, roomId: true, provider: true }).extend({ status: z.enum(["HEALTHY", "PARTIAL_FAILURE", "FAILED", "SYNCING", "NOT_SYNCED", "DISABLED"]).optional().or(z.literal("").transform(() => undefined)) });
