...
export class IcsDocumentParseError extends Error { constructor(message = "ICS 문서 전체를 분석할 수 없습니다.") { super(message); this.name = "IcsDocumentParseError"; } }
...
export function parseIcsCalendar(content: string): CalendarParseResult {
  ...
  if (components.length > 0 && invalidCount / components.length > ICS_MAX_INVALID_EVENT_RATIO) throw new IcsDocumentParseError("유효하지 않은 VEVENT 비율이 허용 범위를 초과했습니다.");
