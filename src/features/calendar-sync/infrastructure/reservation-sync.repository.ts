...
export async function persistReservationSync(input: PersistReservationSyncInput) {
  return prisma.$transaction(async (tx) => {
    ...
    const classification = classifyReservations(existing, input.reservations, {
      ...
      fullyParsed: input.eventCounts.parsedEventCount > 0 && input.eventCounts.failedEventCount === 0,
    });
    ...
    const cancelledCount = explicitCancelledCount + classification.staleCancellationIds.length;
