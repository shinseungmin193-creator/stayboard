...
import { ROOM_CALENDAR_STATUS_META, type RoomCalendarStatus } from "../types/room-calendar-summary";
...
export function RoomCalendarStatusBadge({ status, className, summary }: { status: RoomCalendarStatus; className?: string; summary?: string }) {
  ...
  const badge = <Badge variant="outline" className={cn("gap-1", meta.className, className)}><Icon className={cn("size-3.5", meta.icon === "loading" && "animate-spin")} />{meta.label}</Badge>;
  if (!summary) return badge;
  return <button type="button" title={summary} aria-label={`${meta.label}: ${summary}`} onClick={() => { if (window.matchMedia("(max-width: 1023px)").matches) window.alert(summary); }} className="cursor-help text-left">{badge}</button>;
}
