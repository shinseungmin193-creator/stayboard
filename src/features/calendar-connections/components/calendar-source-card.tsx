...
function SyncStatus({ source }: { source: CalendarSourceSummary }) {
  ...
  if (source.isWarning) return <Badge variant="outline" className="gap-1 border-amber-500/35 bg-amber-500/10 text-amber-800 dark:text-amber-300"><AlertTriangle />주의</Badge>;
    ...
    <Card size="sm" className="overflow-visible">
      ...
      <CardContent className="space-y-4">
        ...
        <dl className="grid grid-cols-2 gap-x-3 gap-y-2 rounded-lg border bg-muted/30 p-3 text-xs sm:grid-cols-3">
          ...
          <div><dt className="text-muted-foreground">예약 분류</dt><dd className="mt-0.5 font-medium tabular-nums">{source.latestReservationEventCount}</dd></div>
          ...
          <div><dt className="text-muted-foreground">파싱 실패</dt><dd className="mt-0.5 font-medium tabular-nums">{source.latestFailedEventCount}</dd></div>
        ...
        {source.isWarning && <div className="flex gap-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 text-xs leading-5 text-amber-800 dark:text-amber-300"><AlertTriangle className="mt-0.5 size-4 shrink-0" /><p>다운로드는 완료됐지만 모든 이벤트가 UNKNOWN 또는 파싱 실패로 제외됐습니다. 동기화 이력에서 분류 근거를 확인해 주세요.</p></div>}
