...
export function RoomCalendarDetailSheet({ room, rooms, open, onOpenChange, canManage, onSourceDeleted, onSourceUpdated }: {room: RoomCalendarSummary | null;rooms: CalendarRoomOption[];open: boolean;onOpenChange: (open: boolean) => void;canManage: boolean;onSourceDeleted: (calendarSourceId: string, message: string) => void;onSourceUpdated: (message: string, warning?: boolean) => void;}) {const locale = useLocale();const formatter = new Intl.DateTimeFormat(locale === "ja" ? "ja-JP" : "ko-KR", { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Tokyo" });const formatDate = (value: Date | null) => value ? formatter.format(new Date(value)) : "-";const i18n = useTranslations();
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="gap-0" style={{ width: "100%", maxWidth: "64rem" }}>{room && <>
    ...
    <Tabs defaultValue="current" className="min-h-0 flex-1 p-4"><TabsList className="w-full"><TabsTrigger value="current">{i18n("auto.m0227")}</TabsTrigger><TabsTrigger value="history">{i18n("auto.m0228")}</TabsTrigger><TabsTrigger value="sources">{i18n("auto.m0229")}</TabsTrigger></TabsList>
      <TabsContent value="current" className="mt-4 space-y-3 overflow-y-auto">
        ...
        {room.sources.length ? room.sources.map((source) => <CalendarSourceCard key={source.id} source={source} rooms={rooms} canManage={canManage} onSourceDeleted={onSourceDeleted} onSourceUpdated={onSourceUpdated} />) : <EmptyState icon={Cable} title={i18n("auto.m0231")} description={i18n("auto.m0232")} />}
      </TabsContent>
      ...
      <TabsContent value="sources" className="mt-4 space-y-3 overflow-y-auto">{room.sources.length ? room.sources.map((source) => <CalendarSourceCard key={source.id} source={source} rooms={rooms} canManage={canManage} onSourceDeleted={onSourceDeleted} onSourceUpdated={onSourceUpdated} />) : <EmptyState icon={Cable} title={i18n("auto.m0231")} description={i18n("auto.m0232")} />}</TabsContent>
    </Tabs>
  </>}</SheetContent></Sheet>;
}
