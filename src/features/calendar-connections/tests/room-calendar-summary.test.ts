...
const log = (calendarSourceId: string, provider: "AIRBNB" | "BOOKING" | "AGODA", status: "SUCCESS" | "FAILED", errorMessage: string | null = null) => ({ calendarSourceId, provider, status, startedAt: new Date("2026-07-24T14:25:00Z"), completedAt: new Date("2026-07-24T14:25:01Z"), fetchedCount: status === "SUCCESS" ? 5 : 0, blockedEventCount: status === "SUCCESS" ? 2 : 0, unknownEventCount: 0, createdCount: status === "SUCCESS" ? 3 : 0, updatedCount: 0, cancelledCount: 0, retryCount: status === "FAILED" ? 2 : 0, httpStatus: status === "FAILED" ? 403 : null, errorCode: status === "FAILED" ? "ICS_HTTP_403" : null, errorMessage, errorDetails: status === "FAILED" ? "technical upstream response" : null, durationMs: 1000 });
