...
interface RunLog { calendarSourceId: string; provider: CalendarProviderType; status: SyncStatus; startedAt: Date; completedAt: Date | null; fetchedCount: number; blockedEventCount: number; unknownEventCount: number; createdCount: number; updatedCount: number; cancelledCount: number; retryCount: number; httpStatus: number | null; errorCode: string | null; errorMessage: string | null; errorDetails: string | null; durationMs: number | null }
...
export function mapRoomCalendarSummary(room: RoomCalendarMapperInput, _now = new Date(), canViewTechnicalDetails = false): RoomCalendarSummary {
  ...
  const status = getRoomCalendarStatus({ activeSourceCount, targetCount: latestRun?.targetCount, successCount: latestRun?.successCount, failedCount: latestRun?.failedCount, running: latestRun?.status === "RUNNING" });
