...
export function mapRoomCalendarSummary(room: RoomCalendarMapperInput, _now = new Date(), canViewTechnicalDetails = false): RoomCalendarSummary {
  ...
  const status = getRoomCalendarStatus({ activeSourceCount, targetCount: latestRun?.targetCount, successCount: latestRun?.successCount, failedCount: latestRun?.failedCount, running: latestRun?.status === "RUNNING", reconnectRequired, warning: latestRun?.syncLogs.some(logHasWarning) });
