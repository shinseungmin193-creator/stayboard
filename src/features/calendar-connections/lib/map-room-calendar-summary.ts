...
export interface RoomCalendarMapperInput { id: string; name: string; propertyId: string; property: { name: string }; calendarSources: Array<{ id: string; provider: CalendarProviderType; name: string; calendarUrl: string; isActive: boolean; connectionStatus: "NORMAL" | "RECONNECT_REQUIRED"; safetyReasonCodes: unknown; lastSyncedAt: Date | null; reservationCounts: { active: number; historical: number; cancelled: number; total: number } }>; syncRuns: MapperRun[]; _count: { reservations: number }; conflicts: Array<{ id: string }> }

function logHealth(log: RunLog, previousSuccessfulReservationEventCount: number | null = null): CalendarSyncHealth {
...
export function mapRoomCalendarSummary(room: RoomCalendarMapperInput, _now = new Date(), canViewTechnicalDetails = false): RoomCalendarSummary {
  ...
  const sources = room.calendarSources.map((source) => {
    ...
    const health = log ? logHealth(log, previousSuccessfulReservationCount(log)) : getCalendarSyncHealth({ status: null, fetchedEventCount: 0, reservationEventCount: 0, blockedEventCount: 0, cancelledEventCount: 0, unknownEventCount: 0, failedEventCount: 0 });
