...
import { CALENDAR_PROVIDER_LABELS, CALENDAR_PROVIDER_TYPES } from "@/providers/calendar/types";

const providers = CALENDAR_PROVIDER_TYPES;
const syncStatuses = ["RUNNING", "SUCCESS", "FAILED", "TIMEOUT"] as const;
const compactStatuses = [{ status: "VACANT", label: "공실" }, { status: "OCCUPIED", label: "투숙 중" }, { status: "CHECK_IN_TODAY", label: "체크인" }, { status: "CHECK_OUT_TODAY", label: "체크아웃" }, { status: "CONFLICT", label: "오버부킹" }] as const;
...
function FilterFields({ properties, filters, showOperational = false }: Pick<ToolbarProps, "properties" | "filters"> & { showOperational?: boolean }) {
  return <>
    <select name="propertyId" defaultValue={filters.propertyId ?? ""} aria-label="숙소 필터" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs"><option value="">모든 숙소</option>{properties.filter((item) => item.isActive).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
    <label className="relative min-w-0"><Search className="absolute left-2.5 top-2 size-3.5 text-muted-foreground" /><input name="query" defaultValue={filters.query ?? ""} aria-label="객실 검색" placeholder="객실 검색" className="h-8 w-full rounded-md border border-input bg-background pl-8 pr-2 text-xs" /></label>
    <select name="status" defaultValue={filters.status ?? ""} aria-label="객실 상태 필터" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs"><option value="">모든 상태</option>{ROOM_OVERVIEW_STATUS_PRIORITY.map((status) => <option key={status} value={status}>{ROOM_OVERVIEW_STATUS_META[status].label}</option>)}</select>
    <select name="provider" defaultValue={filters.provider ?? ""} aria-label="Provider 필터" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs"><option value="">모든 Provider</option>{providers.map((provider) => <option key={provider} value={provider}>{CALENDAR_PROVIDER_LABELS[provider]}</option>)}</select>
    <select name="syncStatus" defaultValue={filters.syncStatus ?? ""} aria-label="동기화 상태 필터" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs"><option value="">모든 동기화</option>{syncStatuses.map((status) => <option key={status}>{status}</option>)}</select>
    {showOperational && <select name="operationalStatus" defaultValue={filters.operationalStatus ?? ""} aria-label="운영 상태 필터" className="h-8 min-w-0 rounded-md border border-input bg-background px-2 text-xs"><option value="">모든 운영 상태</option><option value="NONE">운영 상태 없음</option><option value="CLEANING_REQUIRED">{ROOM_OPERATIONAL_STATUS_META.CLEANING_REQUIRED.label}</option><option value="INSPECTION_REQUIRED">{ROOM_OPERATIONAL_STATUS_META.INSPECTION_REQUIRED.label}</option></select>}
  </>;
}
...
export function RoomOverviewToolbar(props: ToolbarProps) {
  ...
  const mobileSummary: Array<{ label: string; value: number; status?: RoomOverviewStatus }> = [
    { label: "전체", value: props.summary.total },
    ...ROOM_OVERVIEW_STATUS_PRIORITY.slice().reverse().map((status) => ({ label: ROOM_OVERVIEW_STATUS_META[status].label, value: props.summary.statuses[status], status })),
  ];
  ...
  return <>
    ...
    <section aria-label="객실 현황 운영 도구" className="hidden overflow-hidden rounded-lg border bg-card xl:block">
      <div className="flex h-11 items-center gap-2 border-b px-3">
        <h1 className="mr-1 shrink-0 text-sm font-bold">객실 현황</h1>
        <SummaryChip href={statusHref()} label="전체" value={props.summary.total} active={!props.filters.status} />
