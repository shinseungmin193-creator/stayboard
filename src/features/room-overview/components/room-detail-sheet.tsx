...
export function RoomDetailContent({ room, canSync }: {room: RoomOverviewCard;canSync: boolean;}) {const i18n = useTranslations();
  ...
  const detailHref = reservation ? `${reservationsHref}&from=${getReservationDateInput(reservation.startDate)}&to=${getReservationDateInput(reservation.endDate)}` : reservationsHref;
  const nights = reservation ? getReservationNightCount(reservation) : 0;
  ...
  return <>
    <div className="grid grid-cols-2 gap-2 px-4">
      ...
      <div className="rounded-lg border bg-muted/25 p-2.5"><span className="block text-[10px] text-muted-foreground">{i18n("reservation.checkIn")}</span><strong className="mt-1 block text-sm">{reservation ? getReservationDateInput(reservation.startDate) : "-"}</strong></div>
      <div className="rounded-lg border bg-muted/25 p-2.5"><span className="block text-[10px] text-muted-foreground">{i18n("reservation.checkOut")}</span><strong className="mt-1 block text-sm">{reservation ? getReservationDateInput(reservation.endDate) : "-"}</strong></div>
    ...
    <section className="min-h-0 space-y-2 px-4" aria-labelledby="room-detail-reservations">
      ...
      <div className="max-h-48 space-y-1.5 overflow-y-auto">
        {room.reservations.length ? room.reservations.map((item) => {const visual = getProviderVisual(item.provider);return <div key={item.id} className="flex items-center gap-2 rounded-lg border p-2 text-xs"><Badge variant="outline" className={cn("shrink-0", visual.className)}>{item.provider === "OTHER" ? getProviderLabel(item.provider, i18n) : visual.shortLabel}</Badge><span className="min-w-0 flex-1 truncate">{getReservationDisplayName(item, i18n("auto.m0397"))}</span><span className="shrink-0 text-[10px] text-muted-foreground">{getReservationDateInput(item.startDate)?.slice(5).replace("-", "/")}–{getReservationDateInput(item.endDate)?.slice(5).replace("-", "/")}</span></div>;}) : <p className="rounded-lg border border-dashed p-3 text-center text-xs text-muted-foreground">{i18n("auto.m0472")}</p>}
      </div>
    </section>
