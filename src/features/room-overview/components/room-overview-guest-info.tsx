...
import { getReservationDateInput } from "@/features/reservations/reservation-date";
...
export function RoomOverviewGuestInfo({ reservation, guestName, reservationCount, isNextReservation }: RoomOverviewGuestInfoProps) {const locale = useLocale(),localeTag = locale === "ja" ? "ja-JP" : "ko-KR";const dateFormatter = new Intl.DateTimeFormat(localeTag, { month: "numeric", day: "numeric", timeZone: "Asia/Tokyo" });const i18n = useTranslations();const startDateInput = getReservationDateInput(reservation.startDate) ?? "1970-01-01";const endDateInput = getReservationDateInput(reservation.endDate) ?? startDateInput;
  return <div className="space-y-2.5" data-room-overview-next-reservation={isNextReservation || undefined}>
    ...
    <dl className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs xl:grid-cols-3 xl:gap-x-2 xl:gap-y-0">
      ...
      <div data-room-overview-night-count><dt className="text-[11px] text-muted-foreground xl:leading-4">{i18n("auto.m0407")}</dt><dd className="mt-0.5 font-semibold tabular-nums xl:mt-0 xl:text-sm xl:leading-5">{i18n("units.nights", { count: Math.max(0, differenceInCalendarDays(new Date(`${endDateInput}T00:00:00Z`), new Date(`${startDateInput}T00:00:00Z`))) })}</dd></div>
