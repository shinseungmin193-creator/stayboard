...
import { ROOM_INSPECTION_BORDER_CLASS } from "../room-overview-visuals";
import { RoomInspectionButton } from "./room-inspection-button";

export function CompactRoomStatusCard({ room, selectionMode, selected, onActivate, onInspectionActivate }: {
  room: RoomOverviewCard;
  selectionMode: boolean;
  selected: boolean;
  onActivate: () => void;
  onInspectionActivate?: (room: RoomOverviewCard) => void;
}) {
  ...
  const inspectionAction = !selectionMode && onInspectionActivate ? () => onInspectionActivate(room) : undefined;

  return (
    <article
      className={cn(
        "relative flex min-h-[9.25rem] min-w-0 flex-col overflow-hidden rounded-xl border p-0 text-left shadow-sm transition-[border-color,box-shadow,transform] [content-visibility:auto] [contain-intrinsic-size:auto_148px] hover:-translate-y-0.5 hover:shadow-md motion-reduce:transform-none motion-reduce:transition-none",
        status.bodyClass,
    ...
    >
      <button
        ...
        className="flex min-h-0 flex-1 flex-col text-left outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
      >
        <div className={cn("flex min-w-0 items-start gap-1.5 px-2.5 py-2", status.headerClass)}>
          ...
          {selectionMode && <span className={cn("grid size-5 shrink-0 place-items-center rounded-full border", selected ? "border-primary bg-primary text-primary-foreground" : "bg-background text-foreground")}>{selected && <Check className="size-3" />}</span>}
          {!selectionMode && <Badge variant="outline" className={cn("h-5 shrink-0 px-1.5 text-[9px]", status.badgeClass)}>{status.label}</Badge>}
        </div>
      ...
      {hasCardAlerts && <div className="mx-2.5 mt-auto flex min-w-0 flex-wrap items-center gap-1 border-t border-current/10 py-1.5 text-[9px] text-muted-foreground">
        {inspectionRequired && <RoomInspectionButton count={room.pendingMemoCount} onClick={inspectionAction} badgeClassName="px-1.5 text-[10px]" />}
        {sync.error && <span className="flex min-w-0 items-center gap-1 font-medium text-destructive"><Clock3 className="size-3 shrink-0" /><span className="truncate">{sync.label}</span></span>}
        {room.activeConflictCount > 0 && <span className="ml-auto flex shrink-0 items-center gap-0.5 font-semibold text-destructive"><AlertTriangle className="size-3" />{room.activeConflictCount}</span>}
      </div>}
    </article>
  );
}
