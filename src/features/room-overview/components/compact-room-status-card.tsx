  ...
  return (
    ...
    >
      ...
      <div className="mt-2 min-h-8 text-[10px] leading-4">
        {reservation ? <p className="truncate font-medium"><span>{formatMobileRoomDate(reservation.startDate)}</span><span className="mx-1 text-muted-foreground">→</span><span>{formatMobileRoomDate(reservation.endDate)}</span></p> : <p className="text-muted-foreground">예약 없음</p>}
        {reservation && <p className="truncate text-muted-foreground">{room.currentReservation ? "현재 예약" : "다음 예약"} · {room.reservationCount}건</p>}
      </div>

      <div className="mt-1 flex min-h-5 min-w-0 items-center gap-1 overflow-hidden" aria-label="연결된 OTA">
        ...
        {room.providers.length === 0 && <span className="flex items-center gap-1 text-[9px] text-muted-foreground"><WifiOff className="size-3" />연결 없음</span>}
      </div>
