...
export function RoomStatusFilterSheet({ filters, propertyId, properties, onApply, onReset }: {
  ...
  const statusOptions: Array<{ value: MobileRoomFilters["status"]; label: string }> = [
    { value: "ALL", label: i18n("auto.m0244") },
    { value: "RESERVED", label: i18n("common.reservation") },
    { value: "VACANT", label: i18n("reservation.vacant") },
    { value: "CHECK_IN_TODAY", label: i18n("reservation.checkIn") },
    { value: "CHECK_OUT_TODAY", label: i18n("reservation.checkOut") },
    { value: "CLEANING", label: i18n("roomStatus.cleaning") },
    { value: "CONFLICT", label: i18n("reservation.overbooking") },
  ];
  ...
  const applySelect = <Key extends "status" | "ota" | "sync">(key: Key, value: MobileRoomFilters[Key]) => {
    onApply({ ...filters, [key]: value }, selectedPropertyId || undefined);
  };

  const submitSearch = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    onApply({ ...filters, query }, selectedPropertyId || undefined);
    setOpen(false);
  };
  ...
  return <Sheet open={open} onOpenChange={changeOpen}>
    ...
    <SheetContent side="bottom" className="gap-0" aria-label={i18n("auto.m0080")}>
      <SheetHeader className="border-b px-4 py-4 text-left">
        ...
        <SheetDescription>{i18n("roomOverviewFilters.instantDescription")}</SheetDescription>
      </SheetHeader>
      <form onSubmit={submitSearch} className="contents">
        <div className="grid gap-4 overflow-y-auto px-4 py-4 sm:grid-cols-2">
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <input id="mobile-room-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={i18n("auto.m0512")} className={fieldClassName} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="mobile-room-property">{i18n("common.property")}</Label>
            <select id="mobile-room-property" value={selectedPropertyId} onChange={(event) => { const nextPropertyId = event.target.value; setSelectedPropertyId(nextPropertyId); onApply(filters, nextPropertyId || undefined); }} className={fieldClassName}>
          ...
          <div className="space-y-1.5">
            ...
            <select id="mobile-room-status" value={filters.status} onChange={(event) => applySelect("status", event.target.value as MobileRoomFilters["status"])} className={fieldClassName}>
              {statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            </select>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="mobile-room-ota">{i18n("auto.m0470")}</Label>
            <select id="mobile-room-ota" value={filters.ota} onChange={(event) => applySelect("ota", event.target.value as MobileRoomFilters["ota"])} className={fieldClassName}>
          ...
          <div className="space-y-1.5">
            ...
            <select id="mobile-room-sync" value={filters.sync} onChange={(event) => applySelect("sync", event.target.value as MobileRoomFilters["sync"])} className={fieldClassName}>
        ...
        <SheetFooter className="grid grid-cols-2 border-t p-4">
          <Button type="button" variant="outline" onClick={() => { setQuery(""); setSelectedPropertyId(""); onReset(); }}>{i18n("auto.m0403")}</Button>
          <Button type="submit">{i18n("roomOverviewFilters.searchButton")}</Button>
        </SheetFooter>
      </form>
    </SheetContent>
  </Sheet>;
