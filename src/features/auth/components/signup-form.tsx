...
export function SignupForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const [codePreview, setCodePreview] = useState<{ companyName: string; role: "ADMIN" }>();
  ...
  return <form action={submit} className="grid gap-3 sm:grid-cols-2">
    ...
    <fieldset className="grid grid-cols-2 gap-2 sm:col-span-2"><legend className="mb-1.5 text-sm font-medium">가입 유형</legend>{([{ value: "new-company", label: "새 회사 만들기" }, { value: "invitation-code", label: "관리자 초대코드로 가입하기" }] as const).map((item) => <button key={item.value} type="button" aria-pressed={signupType === item.value} onClick={() => { setSignupType(item.value); setMessage(undefined); setCodePreview(undefined); }} className={`min-h-11 rounded-lg border px-3 text-sm font-medium ${signupType === item.value ? "border-primary bg-primary text-primary-foreground" : "bg-background"}`}>{item.label}</button>)}</fieldset>
    {signupType === "new-company" ? <div className="space-y-1.5 sm:col-span-2"><Label htmlFor="signup-company">회사명</Label><Input id="signup-company" name="companyName" required /><p className="text-xs text-muted-foreground">새 Company가 생성되며 가입자는 관리자가 됩니다.</p>{field("companyName") && <p className="text-xs text-destructive">{field("companyName")}</p>}</div> : <div className="space-y-2 sm:col-span-2"><Label htmlFor="signup-code">관리자 초대코드</Label><div className="flex gap-2"><Input id="signup-code" name="invitationCode" autoCapitalize="characters" autoComplete="off" required onChange={() => setCodePreview(undefined)} placeholder="SB-ADMIN-XXXXX-XXXXX-XXXXX-XXXXX" /><Button type="submit" formAction={verify} variant="outline" disabled={checking}>{checking ? <LoaderCircle className="animate-spin" /> : "코드 확인"}</Button></div>{field("invitationCode") && <p className="text-xs text-destructive">{field("invitationCode")}</p>}{codePreview && <div role="status" className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm"><p className="flex items-center gap-1 font-medium text-emerald-700 dark:text-emerald-300"><CheckCircle2 className="size-4" />관리자 초대코드 확인 완료</p><p className="mt-1">회사: {codePreview.companyName}</p><p>역할: 관리자</p></div>}</div>}
