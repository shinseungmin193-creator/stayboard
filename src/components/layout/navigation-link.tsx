...
export function NavigationLink({ label, href, icon: Icon, mobile = false }: { label: string; href: string; icon: LucideIcon; mobile?: boolean }) {
  ...
  if (mobile) return <Link href={href} aria-current={active ? "page" : undefined} className={`flex min-w-0 flex-col items-center justify-center gap-1 px-1 text-[11px] font-medium ${active ? "text-primary" : "text-muted-foreground"}`}><Icon className="size-5" /><span className="w-full truncate text-center">{label}</span></Link>;
