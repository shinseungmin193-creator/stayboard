...
import { AccessDenied, authorizeSidebarMenuAccess, companyScopeIds, getCurrentAccessContext } from "@/features/access-control";
...
export default async function OccupancyStatisticsPage({ searchParams }: {searchParams: Promise<Record<string, string | string[] | undefined>>;}) {const i18n = await getTranslations();
  const context = await getCurrentAccessContext();const access = context ? await authorizeSidebarMenuAccess("occupancy-statistics") : null;if (access && !access.allowed) return <AccessDenied role={access.context?.role ?? null} />;const companyIds = context ? companyScopeIds(context) : undefined;
