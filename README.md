...
Node.js 서버가 실행 중이면 활성 OTA 캘린더를 주기적으로 동기화합니다. 기본값은 5분마다 대상을 확인하고, 마지막 성공 동기화 후 15분이 지난 소스를 갱신하는 방식입니다. OTA iCal은 체크아웃 뒤 과거 이벤트를 제거할 수 있으므로 자동 동기화를 장기간 끄면 그 사이 생성됐다가 종료된 예약은 나중에 복구할 수 없습니다.

주기와 배치 크기는 `.env.example`의 `CALENDAR_AUTO_SYNC_*` 값으로 조정할 수 있습니다. 여러 서버 인스턴스가 동시에 실행돼도 기존 PostgreSQL advisory lock이 같은 캘린더 소스의 중복 반영을 막습니다. OTA feed에서 사라진 이벤트 중 이미 체크아웃이 끝난 예약은 과거 이력으로 보존하고, feed에 명시적으로 `CANCELLED` 또는 비예약 차단 이벤트로 남은 경우만 기존 분류 정책을 적용합니다.
