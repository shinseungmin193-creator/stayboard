...
ssh -p "%VPS_PORT%" "%VPS_USER%@%VPS_HOST%" "bash %REMOTE_TEMP% '%REMOTE_PATH%' '%BRANCH%' '%PM2_APP%'"
set "DEPLOY_EXIT=%ERRORLEVEL%"
goto cleanup_remote

:cleanup_remote
echo [9/10] 원격 임시 파일을 정리합니다.
...
echo "[1/10] 프로젝트 경로와 안전 백업 대상을 확인합니다."
...
echo "[2/10] 최신 코드를 fast-forward 방식으로 반영합니다."
...
echo "[7/10] Build 성공 후 PM2 앱을 재시작합니다."
pm2 restart "$PM2_APP" --update-env
pm2 save

echo "[8/10] PM2 앱 상태를 확인합니다."
pm2 status "$PM2_APP"
